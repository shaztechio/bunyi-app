...
`bunyi.defaults.json` is an immutable distribution file. Windows/Linux and the
native macOS CLI resolve it beside the executable (`AppContext.BaseDirectory`
for .NET); the native macOS app resolves it from the application bundle's
Resources directory. It is separate from per-user settings and backups,
contains no secrets, and is read once per app/server startup:
...
usable; distribution validation rejects missing or invalid files. Every shipped
desktop and CLI package stages and verifies the canonical mirror configuration
from `spec/bunyi.defaults.json`. MSIX packaging must do so without changing the
caller's portable publish folder.
