...
**Buffering audio** means the next part is still being made. Bunyi measures how quickly speech is being generated and adjusts the amount of audio it prepares before starting or resuming playback. The minimum is 10 seconds; slower generation needs a larger head start to reduce interruptions. Very slow runs may wait until most or all of the recording is ready. When generation finishes, any shorter remainder plays immediately. Unexpected slowdowns can still cause a pause. The completed recording does not contain these waiting gaps, and it may have a slightly different volume from the preview. A long preview does not play again automatically when its file is saved.
