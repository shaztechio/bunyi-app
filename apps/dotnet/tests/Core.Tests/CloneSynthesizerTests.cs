...
public sealed class CloneSynthesizerTests : IDisposable
{
    ...
    [Fact]
    public async Task Preview_callback_reaches_clone_pipeline()
    {
        var pipeline = new FakePipeline();
        await using var synth = await LoadedAsync(pipeline);
        AudioPreviewChunk? chunk = null;
        await synth.SynthesizeAsync(Request(audio: MakeClip()) with { AudioPreview = value => chunk = value }, default);
